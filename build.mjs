...
const lines = projects.map(p => `| ${p.name} | ${p.path === 'gameperf' ? '研发工作台演示 + 后端源码' : p.demo ? '交互原型' : '代码目录'} | ${p.demo ? `[在线演示](https://arsenaltj.github.io/creative-hub/projects/${p.path}/)` : '—'} | [源码](${repo(p)}) |`).join('\n');
