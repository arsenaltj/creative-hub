# VibeDock Companion 0.4

Windows PC 伴侣服务 + 圆形吧唧模拟器。PC 顶部可以切换 Codex / WorkBuddy；圆屏四区中央打开工具选择页，选择另一个工具后才切换。圆屏内只展示当前工具的四个任务。

PC 持续读取 Codex 本地生命周期事件与 WorkBuddy 桌面状态库，模拟设备通过 HTTP + SSE 同步。0.4 增加首次使用连接检查、未上屏任务的显式换槽、最新回复节选，以及 Windows 用户分区偏好保存。实际批准/拒绝继续在原客户端处理，原生语音触发尚未接入。
