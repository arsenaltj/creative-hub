...
function apply(next){
 ...
 if(state && state.epoch!==next.epoch){decision=null;outputView=null;pendingPinTaskId=null;$('confirmDialog').close();screen='tasks'}
...
function renderGuide(){
 $('firstRun').hidden=!guideVisible || deviceOnly;
 if(!guideVisible || !state)return;
 const integration=name=>state.integrations?.[name];
 const checks=[
  ['PC 服务',online?'正在运行':'连接中断',online],
  ['Codex',integration('codex')?.mode==='demo'?'模拟数据':integration('codex')?.connected?'任务状态可读':'真实状态尚不可读',integration('codex')?.connected && integration('codex')?.mode==='live'],
  ['WorkBuddy',integration('workbuddy')?.mode==='demo'?'模拟数据':integration('workbuddy')?.connected?'任务状态可读':'真实状态尚不可读',integration('workbuddy')?.connected && integration('workbuddy')?.mode==='live'],
  ['物理吧唧','尚未接入；下方是交互模拟器',false],
 ];
 $('setupChecks').innerHTML=checks.map(([name,note,good])=>`<div class="setup-check ${good?'good':'wait'}"><b>${good?'✓':'·'} ${esc(name)}</b><small>${esc(note)}</small></div>`).join('');
}
function render(){
 if(!state)return;
...
function renderScreen(){
 ...
 if(screen==='slot-picker'){
  ...
  content=`<div class="device-kicker">替换一个分区</div><h2 class="output-title">${esc(target?.title || '任务已移出列表')}</h2><div class="device-list">${s.tasks.map((v,i)=>`<button data-pin-slot="${i}" ${disabled || !target?'disabled':''}><span><b>0${i+1} · ${esc(v?.title || '空闲')}</b><small>${v?'替换这一项':'放入空闲位'}</small></span><span>›</span></button>`).join('')}</div>`;
 ...
 if(screen==='output'){
  ...
  const pages=outputPages(output?.text),page=Math.min(output?.page || 0,pages.length-1);
  if(output)output.page=page;
  const hasNew=!!(t?.output?.text && t.output.text!==output?.text);
  content=`<div class="device-kicker">最新回复节选</div><h2 class="output-title">${esc(t?.title || '任务已变更')}</h2><p class="output-page">${esc(pages[page])}</p><div class="output-nav"><button data-output-prev ${page===0?'disabled':''}>上一页</button><button data-output-next ${page===pages.length-1?'disabled':''}>下一页</button></div><span class="output-count">${page+1} / ${pages.length}${Array.from(output?.text || '').length>800?' · 已截取':''}</span>${hasNew?'<button class="output-refresh" data-output-refresh>有新输出 · 查看</button>':''}`;
  $('screenHint').textContent='阅读时不自动跳页；完整回复、代码和 diff 请在电脑端原会话查看。';
 }
 if(screen==='attention'){
...
async function placeTask(slot,source=pendingPinSource){
 const taskId=pendingPinTaskId;if(!taskId)return;
 await send('task.pin',{taskId,slot},source);
 pendingPinTaskId=null;$('slotDialog').close();screen='detail';renderScreen();
}
async function chooseTask(taskId,source){
 ...
 if(empty>=0){await placeTask(empty,source);return}
 if(source==='device'){screen='slot-picker';renderScreen();return}
 $('slotTaskTitle').textContent=s.availableTasks.find(t=>t.id===taskId)?.title || '选择要替换的任务';
...
document.addEventListener('click',async e=>{
 ...
 if(b.dataset.open){const source=b.dataset.open,t=current(source==='device'?badgeState:state);b.disabled=true;try{const result=await send('task.open',{taskId:t.id},source);toast(result.message)}finally{renderKey='';render()}}
  ...
  if(b.hasAttribute('data-output-refresh')){const t=current(badgeState);if(t?.output?.text){outputView={taskId:t.id,text:t.output.text,page:0};renderScreen()}}
